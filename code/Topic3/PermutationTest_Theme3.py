...
count=0
samples=input("enter number of samples") #Numebr of Permutations
dup=samples
x1=[]
x2=[]
...
def calculate():
	...
	with open('processed_data.csv','r') as inf:
        ...
        with open('processed_data.csv','r') as f:
                ...
                for row in reader:
                        ...
                        if(float(row[32]) >  accept[0] and float(row[32]) <  accept[1]): #Removing Outliers
                                if(row[5]=='Police Administrator II'):
                                        list1.append(float(row[32])) # Police Administrator II costs will be in this list
		...
		print(pvalue)
		if(pvalue<=0.05): #Comparing  pvalue with 95% CI alpha value
			print("Rejecting NUll Hypothesis --- Distributions Police Administrator II and Fire Administrator are Different")
		else:
			print("Accepting Null Hypothesis --- Distributions POlice Administrator II and Fire Administrator are Identical")
